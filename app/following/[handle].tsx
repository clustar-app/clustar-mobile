...
// Following list — accounts that @handle follows. Same shape as followers.
...
export default function FollowingScreen() {
  ...
  const q = useQuery({
    ...
    queryFn: () => userApi.getFollowing(accessToken!, handle),
    ...
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <View style={styles.topBar}>
        ...
        <Text style={styles.topTitle}>
          {q.data ? `${q.data.length} following` : "Following"}
        </Text>
        <View style={{ width: 36 }} />
      </View>
          ...
          renderItem={({ item }) => (
            <UserListRow
              ...
              onFollowChange={(isFollowing) => {
                queryClient.setQueryData<FollowUser[]>(["following", handle], prev =>
